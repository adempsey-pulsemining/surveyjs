<template>
  <div class="sv_q_rating">
    <div>
      <label v-for="(item, index) in question.visibleRateValues" :key="item.value" :class="getCss(question, item)">
        <input type="radio"
               ...
               @change="change"
               v-bind:aria-label="item.locText.text"
        />
    ...
    <survey-other-choice v-show="question.hasOther" :question="question"></survey-other-choice>
  </div>
</template>
...
export default {
  props: {
    question: Object as () => QuestionRatingModel
  },
  methods: {
    getCss(question: QuestionRatingModel, item: any) {
      let css = "sv_q_rating_item";
      return question.value == item.value ? css + " active" : css;
    },
    change(e: any) {
